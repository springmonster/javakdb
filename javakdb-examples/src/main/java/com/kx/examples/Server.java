...
public class Server {
    ...
    private static class ServerC extends c implements AutoCloseable {
        ServerC(ServerSocket s) throws java.io.IOException {
            super(s);
        }

        @Override
        public void w(int i, Object o) throws java.io.IOException {
            super.w(i, o);
        }
    ...
    public static void main(String[] args) {
        ...
        try (ServerC c = new ServerC(new ServerSocket(port))) {
            ...
            while (ok) {
                ...
                switch ((byte) msg[0]) {
                    case 0:
                        LOGGER.log(Level.SEVERE, "Discarding async message");
                        break;
                    case 1:
                        c.kr(msg[1]);
                        break; // sync request, echo msg back to remote
                    default:
                        LOGGER.log(Level.SEVERE, "Unrecognized msg type: {0}. Quitting....", msg[0]);
                        ok = false;
                        break;
                }
            }
